...
const form = document.querySelector('.container .lorem-form');
const amount = form.querySelector('input');
const paragraphsContainer = document.querySelector('.container .lorem-text');
form.addEventListener('submit', (e) => {
    ...
    if (isNaN(amountValue) || amountValue <= 0 || amountValue > 9) {
        paragraphsContainer.innerHTML = createParagraphs(texts[random]);
        return;
    }
    let paragraphList = '';
    for (let i = 0; i < amountValue; i++) {
        paragraphList += createParagraphs(texts[i]);
    }
    paragraphsContainer.innerHTML = paragraphList;
});
